...
struct RiveHedgehogAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "hedgehog_v1", autoPlay: true)
    ...
    static let hedgehogBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let hedgehogBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "C4956A", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = hedgehogBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
