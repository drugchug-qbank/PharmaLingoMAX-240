...
struct RiveDragonAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "dragon_v1", autoPlay: true)
    ...
    static let dragonBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let dragonBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "66BB6A", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = dragonBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
