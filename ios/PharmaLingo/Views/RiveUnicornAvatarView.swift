...
struct RiveUnicornAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "unicorn_v1", autoPlay: true)
    ...
    static let unicornBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let unicornBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "F8BBD0", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = unicornBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
