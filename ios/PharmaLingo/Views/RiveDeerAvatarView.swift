...
struct RiveDeerAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "deer_v1", autoPlay: true)
    ...
    static let deerBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let deerBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "C4956A", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = deerBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
