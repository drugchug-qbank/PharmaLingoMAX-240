...
struct RiveTigerAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "tiger_v1", autoPlay: true)
    ...
    static let tigerBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let tigerBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "FF8C00", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = tigerBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
