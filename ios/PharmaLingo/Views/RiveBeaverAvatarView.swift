...
struct RiveBeaverAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "beaver_v1", autoPlay: true)
    ...
    static let beaverBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let beaverBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "8B5E3C", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = beaverBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
