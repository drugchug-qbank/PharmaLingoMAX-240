...
struct RivePandaAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "panda_v1", autoPlay: true)
    ...
    static let pandaBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let pandaBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "E0E0E0", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = pandaBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
