...
struct RiveLionAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "lion_v1", autoPlay: true)
    ...
    static let lionBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let lionBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "FFA726", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = lionBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
