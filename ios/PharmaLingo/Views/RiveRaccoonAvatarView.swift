...
struct RiveRaccoonAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "raccoon_v1", autoPlay: true)
    ...
    static let raccoonBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let raccoonBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "78909C", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = raccoonBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
