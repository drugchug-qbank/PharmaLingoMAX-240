...
struct DrugDataService {
    ...
    private init() {
        ...
        for mod in self.modules {
            ...
            for sub in mod.subsections where sub.isMasteryQuiz {
                ...
                if sub.id == Self.masteringGUBoneHormonesPreventionSubsectionId {
                    bank[sub.id]?.append(contentsOf: Self.curatedAddOnQuestions_10eMastery)
                }
            }
        }
    ...
    static func populateCuratedBank(_ bank: inout [String: [Question]]) {
        ...
        bank["5a"]?.append(contentsOf: Self.curatedAddOnQuestions_5a_supplement)

        if bank["5b"] == nil { bank["5b"] = [] }
        bank["5b"]?.append(contentsOf: Self.curatedAddOnQuestions_5b)
        bank["5b"]?.append(contentsOf: Self.curatedAddOnQuestions_5b_supplement)

        if bank["5c"] == nil { bank["5c"] = [] }
        bank["5c"]?.append(contentsOf: Self.curatedAddOnQuestions_5c)
        bank["5c"]?.append(contentsOf: Self.curatedAddOnQuestions_5c_supplement)

        if bank["5d"] == nil { bank["5d"] = [] }
        bank["5d"]?.append(contentsOf: Self.curatedAddOnQuestions_5d)
        bank["5d"]?.append(contentsOf: Self.curatedAddOnQuestions_5d_supplement)

        if bank["6b"] == nil { bank["6b"] = [] }
        bank["6b"]?.append(contentsOf: Self.curatedAddOnQuestions_6b)
        bank["6b"]?.append(contentsOf: Self.module6ContrastQuestions.filter { $0.subsectionId == "6b" })

        if bank["6c"] == nil { bank["6c"] = [] }
        bank["6c"]?.append(contentsOf: Self.curatedAddOnQuestions_6c)
        bank["6c"]?.append(contentsOf: Self.module6ContrastQuestions.filter { $0.subsectionId == "6c" })

        if bank["6d"] == nil { bank["6d"] = [] }
        bank["6d"]?.append(contentsOf: Self.curatedAddOnQuestions_6d)
        bank["6d"]?.append(contentsOf: Self.module6ContrastQuestions.filter { $0.subsectionId == "6d" })

        if bank["6e"] == nil { bank["6e"] = [] }
        bank["6e"]?.append(contentsOf: Self.curatedAddOnQuestions_6e)
        bank["6e"]?.append(contentsOf: Self.curatedAddOnQuestions_6e_supplement)
        bank["6e"]?.append(contentsOf: Self.module6ContrastQuestions.filter { $0.subsectionId == "6e" })

        if bank["7b"] == nil { bank["7b"] = [] }
        bank["7b"]?.append(contentsOf: Self.curatedAddOnQuestions_7b)
        bank["7b"]?.append(contentsOf: Self.curatedAddOnQuestions_7b_supplement)
        bank["7b"]?.append(contentsOf: Self.curatedContrastQuestions_7.filter { $0.subsectionId == "7b" })

        if bank["7c"] == nil { bank["7c"] = [] }
        bank["7c"]?.append(contentsOf: Self.curatedAddOnQuestions_7c)
        bank["7c"]?.append(contentsOf: Self.curatedAddOnQuestions_7c_supplement)
        bank["7c"]?.append(contentsOf: Self.curatedContrastQuestions_7.filter { $0.subsectionId == "7c" })

        if bank["7d"] == nil { bank["7d"] = [] }
        bank["7d"]?.append(contentsOf: Self.curatedAddOnQuestions_7d)
        bank["7d"]?.append(contentsOf: Self.curatedAddOnQuestions_7d_supplement)
        bank["7d"]?.append(contentsOf: Self.curatedContrastQuestions_7.filter { $0.subsectionId == "7d" })

        if bank["7e"] == nil { bank["7e"] = [] }
        bank["7e"]?.append(contentsOf: Self.curatedAddOnQuestions_7e)
        bank["7e"]?.append(contentsOf: Self.curatedAddOnQuestions_7e_supplement)
        bank["7e"]?.append(contentsOf: Self.curatedContrastQuestions_7.filter { $0.subsectionId == "7e" })

        if bank["8a"] == nil { bank["8a"] = [] }
        bank["8a"]?.append(contentsOf: Self.curatedAddOnQuestions_8a)
        bank["8a"]?.append(contentsOf: Self.curatedAddOnQuestions_8a_supplement)

        if bank["8b"] == nil { bank["8b"] = [] }
        bank["8b"]?.append(contentsOf: Self.curatedAddOnQuestions_8b)
        bank["8b"]?.append(contentsOf: Self.curatedAddOnQuestions_8b_supplement)

        if bank["8c"] == nil { bank["8c"] = [] }
        bank["8c"]?.append(contentsOf: Self.curatedAddOnQuestions_8c)
        bank["8c"]?.append(contentsOf: Self.curatedAddOnQuestions_8c_supplement)

        if bank["8d"] == nil { bank["8d"] = [] }
        bank["8d"]?.append(contentsOf: Self.curatedAddOnQuestions_8d)
        bank["8d"]?.append(contentsOf: Self.curatedAddOnQuestions_8d_supplement)

        if bank["8e"] == nil { bank["8e"] = [] }
        bank["8e"]?.append(contentsOf: Self.curatedAddOnQuestions_8e)
        bank["8e"]?.append(contentsOf: Self.curatedAddOnQuestions_8e_supplement)

        if bank["9b"] == nil { bank["9b"] = [] }
        bank["9b"]?.append(contentsOf: Self.curatedAddOnQuestions_9b)
        bank["9b"]?.append(contentsOf: Self.curatedAddOnQuestions_9b_supplement)

        if bank["9c"] == nil { bank["9c"] = [] }
        bank["9c"]?.append(contentsOf: Self.curatedAddOnQuestions_9c)
        bank["9c"]?.append(contentsOf: Self.curatedAddOnQuestions_9c_supplement)

        if bank["9d"] == nil { bank["9d"] = [] }
        bank["9d"]?.append(contentsOf: Self.curatedAddOnQuestions_9d)
        bank["9d"]?.append(contentsOf: Self.curatedAddOnQuestions_9d_supplement)

        if bank["9e"] == nil { bank["9e"] = [] }
        bank["9e"]?.append(contentsOf: Self.curatedAddOnQuestions_9e)
        bank["9e"]?.append(contentsOf: Self.curatedAddOnQuestions_9e_supplement)
        ...
        for q in Self.module10ContrastQuestions {
            if bank[q.subsectionId] == nil { bank[q.subsectionId] = [] }
            bank[q.subsectionId]?.append(q)
        }
    }
}
