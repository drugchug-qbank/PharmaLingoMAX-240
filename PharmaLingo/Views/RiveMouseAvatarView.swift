...
struct RiveMouseAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "mouse_v1", autoPlay: true)
    ...
    static let mouseBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let mouseBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "B0BEC5", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = mouseBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
