...
struct RiveCatAvatarView: View {
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "cat_v1", autoPlay: true)
    ...
    static let catBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let catBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "FF8A65", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = catBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
