...
struct RiveMonkeyAvatarView: View {
    let size: CGFloat
    ...
    @State private var riveViewModel = RiveViewModel(fileName: "monkey_v1", autoPlay: true)
    ...
    static let monkeyBodyColors: [(name: String, index: Double)] = [
        ("Base", 0),
        ("Cocoa Cream", 1),
        ("Charcoal Mint", 2),
        ("Honey Gold", 3),
    ...
    static let monkeyBodyColorHexes: [(name: String, hex: String, index: Double)] = [
        ("Base", "A1887F", 0),
    ...
    static func bodyColorIndex(for hex: String) -> Double {
        ...
        if let match = monkeyBodyColorHexes.first(where: { $0.hex.uppercased() == upperHex }) {
            return match.index
        }
        return 0
    }
